...
function array= genPointMassFlatSideCyl(mass, width, radius, thickness, ...
					nxpoints, nypoints)

	xgrid = thickness     / nxpoints;
	ygrid = radius * 2.0 / nypoints;
	zgrid = ygrid;
	nzpoints= nypoints;
    
    h=radius-width/2;

	density = mass / ( (pi * ( radius^2) * thickness) ...
        -(radius^2*acos((radius-h)/radius)-(radius-h)*sqrt(2*radius*h-h^2))* thickness);
	...
	for i = 1:nxpoints
		for j = 1:nypoints
			for k = 1:nzpoints
				...
				if(pRadius <= radius && abs(newMass(3)) <= width/2)
